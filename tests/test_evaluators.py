...
class TestEventHappenedExactly:
    def test_returns_created_at_on_exact_count(self):
        ...
        assert event_happened_exactly(1, 3, events, FIELD_MAP) == Decision(True, dt(3))

    def test_returns_false_when_count_differs(self):
        events = [make_event(1, dt(1)), make_event(1, dt(2))]
        assert event_happened_exactly(1, 3, events, FIELD_MAP) == Decision(False)

    def test_returns_false_when_no_events(self):
        assert event_happened_exactly(1, 1, [], FIELD_MAP) == Decision(False)


class TestEventHappenedFewerThan:
    def test_true_when_fewer(self):
        events = [make_event(1), make_event(1)]
        assert event_happened_fewer_than(1, 3, events, FIELD_MAP) == Decision(True)

    def test_false_when_equal(self):
        events = [make_event(1), make_event(1), make_event(1)]
        assert event_happened_fewer_than(1, 3, events, FIELD_MAP) == Decision(False)
...
class TestEventHappenedAtLeast:
    def test_true_when_equal(self):
        ...
        assert event_happened_at_least(1, 3, events, FIELD_MAP) == Decision(True)
    ...
    def test_false_when_fewer(self):
        events = [make_event(1), make_event(1)]
        assert event_happened_at_least(1, 3, events, FIELD_MAP) == Decision(False)
...
class TestEventRevoked:
    def test_returns_revoked_at(self):
        ...
        assert event_revoked(1, events, FIELD_MAP) == Decision(True, revoked_at)

    def test_returns_false_when_not_revoked(self):
        events = [make_event(1)]
        assert event_revoked(1, events, FIELD_MAP) == Decision(False)

    def test_returns_false_when_no_events(self):
        assert event_revoked(1, [], FIELD_MAP) == Decision(False)
    ...
    def test_returns_last_revoked_at_when_multiple(self):
        ...
        assert event_revoked(1, events, FIELD_MAP) == Decision(True, dt(5))


class TestDelayPassed:
    def test_satisfied_once_threshold_reached(self):
        created_at = dt(1)
        events = [make_event(1, created_at=created_at)]
        threshold = created_at + datetime.timedelta(days=7)
        now = threshold
        assert delay_passed(1, 7, events, FIELD_MAP, now) == Decision(True, threshold)

    def test_satisfied_after_threshold(self):
        created_at = dt(1)
        events = [make_event(1, created_at=created_at)]
        threshold = created_at + datetime.timedelta(days=7)
        now = threshold + datetime.timedelta(days=1)
        assert delay_passed(1, 7, events, FIELD_MAP, now) == Decision(True, threshold)

    def test_not_yet_satisfied_before_threshold(self):
        """The actual fix: a future threshold must not be treated as satisfied."""
        created_at = dt(1)
        events = [make_event(1, created_at=created_at)]
        threshold = created_at + datetime.timedelta(days=90)
        now = created_at + datetime.timedelta(days=83)
        result = delay_passed(1, 90, events, FIELD_MAP, now)
...
class TestPayloadMatch:
    def test_exact_match_returns_created_at(self):
        ...
        assert payload_match("q", "yes", {}, events, FIELD_MAP, type_id=1) == Decision(True, created_at)

    def test_no_match_returns_false(self):
        events = [make_event(1, q="no")]
        assert payload_match("q", "yes", {}, events, FIELD_MAP, type_id=1) == Decision(False)
    ...
    def test_no_event_returns_false(self):
        assert payload_match("q", "yes", {}, [], FIELD_MAP, type_id=1) == Decision(False)

    def test_none_answer_match(self):
        created_at = dt(1)
        events = [make_event(1, created_at=created_at, q=None)]
        assert payload_match("q", None, {}, events, FIELD_MAP, type_id=1) == Decision(True, created_at)

    def test_sub_type_gt(self):
        created_at = dt(1)
        events = [make_event(1, created_at=created_at, score="10")]
        assert payload_match("score", "5", {"sub_type": "gt"}, events, FIELD_MAP, type_id=1) == Decision(True, created_at)
    ...
    def test_uses_most_recent_event(self):
        ...
        assert payload_match("q", "new", {}, events, FIELD_MAP, type_id=1) == Decision(True, newer)
    ...
    def test_no_type_id_checks_last_event_of_any_type(self):
        ...
        assert payload_match("total", "10", {}, events, FIELD_MAP) == Decision(True, created_at)
    ...
    def test_no_type_id_ignores_revoked(self):
        ...
        assert payload_match("total", "5", {}, events, FIELD_MAP) == Decision(True, dt(1))

    def test_no_events_with_no_type_id_returns_false(self):
        assert payload_match("total", "10", {}, [], FIELD_MAP) == Decision(False)
...
class TestPayloadMatchSeqNum:
    def test_seq_num_0_is_most_recent(self):
        ...
        assert payload_match("q", "new", {}, events, FIELD_MAP, type_id=1, seq_num=0) == Decision(True, dt(2))

    def test_seq_num_1_is_second_most_recent(self):
        events = [make_event(1, created_at=dt(1), q="old"), make_event(1, created_at=dt(2), q="new")]
        assert payload_match("q", "old", {}, events, FIELD_MAP, type_id=1, seq_num=1) == Decision(True, dt(1))

    def test_seq_num_out_of_range_returns_false(self):
        events = [make_event(1, q="yes")]
        assert payload_match("q", "yes", {}, events, FIELD_MAP, type_id=1, seq_num=5) == Decision(False)
    ...
    def test_seq_num_skips_revoked_events(self):
        ...
        assert payload_match("q", "c", {}, events, FIELD_MAP, type_id=1, seq_num=0) == Decision(True, dt(4))
        assert payload_match("q", "a", {}, events, FIELD_MAP, type_id=1, seq_num=1) == Decision(True, dt(1))
    ...
    def test_seq_num_1_without_type_id(self):
        ...
        assert payload_match("total", "5", {}, events, FIELD_MAP, seq_num=1) == Decision(True, dt(1))


class TestLastEventTypeEquals:
    def test_most_recent_matches(self):
        events = [make_event(1, dt(1)), make_event(2, dt(2))]
        assert last_event_type_equals(2, 0, events, FIELD_MAP) == Decision(True, dt(2))

    def test_most_recent_does_not_match(self):
        events = [make_event(1, dt(1)), make_event(2, dt(2))]
        assert last_event_type_equals(1, 0, events, FIELD_MAP) == Decision(False)

    def test_second_most_recent(self):
        events = [make_event(1, dt(1)), make_event(2, dt(2))]
        assert last_event_type_equals(1, 1, events, FIELD_MAP) == Decision(True, dt(1))

    def test_index_out_of_range_returns_false(self):
        events = [make_event(1)]
        assert last_event_type_equals(1, 5, events, FIELD_MAP) == Decision(False)

    def test_no_events_returns_false(self):
        assert last_event_type_equals(1, 0, [], FIELD_MAP) == Decision(False)


class TestAvailableOnDateRange:
    def test_within_range_returns_satisfied_with_start_date(self):
        start = "2025-01-10"
        end = "2025-01-31"
        now = datetime.datetime(2025, 1, 15, tzinfo=ZoneInfo("UTC"))
        result = available_on_date_range(start, end, 0, now)
        assert result == Decision(True, datetime.datetime(2025, 1, 10, tzinfo=ZoneInfo("UTC")))

    def test_before_range_is_not_yet_satisfied(self):
        """Not yet satisfied before start, but predicts the start date."""
        start = "2025-01-15"
        end = "2025-01-31"
        now = datetime.datetime(2025, 1, 5, tzinfo=ZoneInfo("UTC"))
        result = available_on_date_range(start, end, 0, now)
        expected_start = datetime.datetime(2025, 1, 15, tzinfo=ZoneInfo("UTC"))
        assert result == Decision(False, expected_start)
        assert result.satisfied is False

    def test_after_range_returns_false_with_no_prediction(self):
        start = "2025-01-01"
        end = "2025-01-10"
        now = datetime.datetime(2025, 1, 20, tzinfo=ZoneInfo("UTC"))
        assert available_on_date_range(start, end, 0, now) == Decision(False, None)

    def test_no_start_date_returns_true(self):
        end = "2025-12-31"
        now = datetime.datetime(2025, 6, 1, tzinfo=ZoneInfo("UTC"))
        assert available_on_date_range(None, end, 0, now) == Decision(True, None)
    ...
    def test_no_end_date_always_available(self):
        ...
        assert result == Decision(True, datetime.datetime(2025, 1, 1, tzinfo=ZoneInfo("UTC")))
...
class TestTakenRecently:
    def test_taken_within_window_returns_created_at(self):
        ...
        assert taken_recently(1, "days", 7, events, FIELD_MAP, now) == Decision(True, created_at)

    def test_taken_outside_window_returns_false(self):
        created_at = dt(1)
        events = [make_event(1, created_at=created_at)]
        now = created_at + datetime.timedelta(days=8)
        assert taken_recently(1, "days", 7, events, FIELD_MAP, now) == Decision(False, None)

    def test_hours_duration_type(self):
        created_at = dt(1)
        events = [make_event(1, created_at=created_at)]
        now = created_at + datetime.timedelta(hours=2)
        assert taken_recently(1, "hours", 3, events, FIELD_MAP, now) == Decision(True, created_at)

    def test_no_events_returns_false(self):
        assert taken_recently(1, "days", 7, [], FIELD_MAP) == Decision(False)
